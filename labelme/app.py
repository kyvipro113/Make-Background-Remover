...
from label_file import set_mask_refine_state, set_gray_scale_state
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        ## Refine mask
        refineMaskMode = QtWidgets.QWidgetAction(self)
        refineMaskMode.setDefaultWidget(QtWidgets.QWidget())
        refineMaskMode.defaultWidget().setLayout(QtWidgets.QVBoxLayout())
        #
        refineMaskLabel = QtWidgets.QLabel(self.tr("Refinement mask with AI"))
        refineMaskLabel.setAlignment(QtCore.Qt.AlignCenter)
        refineMaskMode.defaultWidget().layout().addWidget(refineMaskLabel)
        #
        self.refineMaskButton = QtWidgets.QPushButton()
        if MainWindow.REFINE_MASK_AI:
            self.refineMaskButton.setText("Disable Refine")
            self.refineMaskButton.setStyleSheet("background-color: red; color: white;")
            self.refineMaskAI = True
        else:
            self.refineMaskButton.setText("Enable Refine")
            self.refineMaskButton.setStyleSheet("background-color: green; color: white;")
            self.refineMaskAI = False
        refineMaskMode.defaultWidget().layout().addWidget(self.refineMaskButton)
        self.refineMaskButton.clicked.connect(self.on_reinfe_mask_button_click)
        
        ##

        ## Gray Scale
        grayScaleMode = QtWidgets.QWidgetAction(self)
        grayScaleMode.setDefaultWidget(QtWidgets.QWidget())
        grayScaleMode.defaultWidget().setLayout(QtWidgets.QVBoxLayout())
        #
        grayScaleLabel = QtWidgets.QLabel(self.tr("Gray scale image output"))
        grayScaleLabel.setAlignment(QtCore.Qt.AlignCenter)
        grayScaleMode.defaultWidget().layout().addWidget(grayScaleLabel)
        #
        self.grayScaleButton = QtWidgets.QPushButton()
        if MainWindow.GRAY_SCALE:
            self.grayScaleButton.setText("Disable Gray Scale")
            self.grayScaleButton.setStyleSheet("")
            self.grayScaleOut = True
        else:
            self.grayScaleButton.setText("Enable Gray Scale")
            self.grayScaleButton.setStyleSheet("background-color: gray; color: white;")
            self.grayScaleOut = False
        grayScaleMode.defaultWidget().layout().addWidget(self.grayScaleButton)
        self.grayScaleButton.clicked.connect(self.on_gray_scale_button_click)
        ...
        # Application state.importDirImagope
        ...
        self.settings = QtCore.QSettings("MK Remove Background Manual", "MK Remove Background Manual")
    ...
    ### SET FLAG REFINE
    def on_reinfe_mask_button_click(self):
        if(self.refineMaskAI):
            self.refineMaskAI = False
            self.refineMaskButton.setStyleSheet("background-color: green; color: white;")
            self.refineMaskButton.setText("Enable Refinement")
            MainWindow.CONTENT_CONFIG['DEEPLEARNING_REFINE_MASK']['USE_MASK_REFINE'] = False
        else: 
            self.refineMaskAI = True
            self.refineMaskButton.setStyleSheet("background-color: red; color: white;")
            self.refineMaskButton.setText("Disable Refinement")
            MainWindow.CONTENT_CONFIG['DEEPLEARNING_REFINE_MASK']['USE_MASK_REFINE'] = True
        with open('config.yaml', 'w') as file:
            yaml.safe_dump(MainWindow.CONTENT_CONFIG, file, sort_keys=False)
        set_mask_refine_state(use_mask_refine=self.refineMaskAI)

    ### SET FLAG GRAY SCALE OUT
    def on_gray_scale_button_click(self):
        if(self.grayScaleOut):
            self.grayScaleOut = False
            self.grayScaleButton.setText("Enable Gray Scale")
            self.grayScaleButton.setStyleSheet("background-color: gray; color: white;")
            MainWindow.CONTENT_CONFIG['GRAY_SCALE'] = False
        else:
            self.grayScaleOut = True
            self.grayScaleButton.setText("Disable Gray Scale")
            self.grayScaleButton.setStyleSheet("")
            MainWindow.CONTENT_CONFIG['GRAY_SCALE'] = True
        with open('config.yaml', 'w') as file:
            yaml.safe_dump(MainWindow.CONTENT_CONFIG, file, sort_keys=False)
        set_gray_scale_state(gray_scale=self.grayScaleOut)
